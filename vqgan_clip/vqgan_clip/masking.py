...
class ClampWithGrad(torch.autograd.Function):
    ...
    def backward(ctx, grad_in):
        ...
        return grad_in * (grad_in * (input - input.clamp(ctx.min, ctx.max)) >= 1e-4), None, None

class HingeWithGrad(torch.autograd.Function):
    @staticmethod
    ...
    def backward(ctx,grad_in):
        input, = ctx.saved_tensors
        return (grad_in*(input-input.clamp(min=0.0)).pow(2)), None, None

replace_grad = ReplaceGrad.apply
clamp_with_grad = torch.clamp #ClampWithGrad.apply
hinge_with_grad = HingeWithGrad.apply
